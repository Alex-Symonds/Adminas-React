...
function WarningMessageSpan(props){
    ...
    return <span class="warning-msg">
                ...
                <span class="msg">{ props.message }</span>
            </span>
}
...
function JobItemsProductMembersUI(props){
    return [
        <div class="subsection product">
            <div class="subsection_heading">{ props.title }</div>
            <div class="ji-list">
                <table>
                    {
...
function JobItemsCreator(props){
    ...
    const KEY_PAIRS = [
        ...
        { be: 'job',            fe:'job_id',        init: `${props.job_id}`  },
    ]
    const {
        formSetForms,
        actionsFormSet,
        backendError,
        handleSubmit,
    } = useFormSet(QUERY_KEY, KEY_PAIRS, props.actions_items, props.editor, MAX_FORMS);
            ...
            <div className={"jobItemsCreator"}>
                ...
                <div className={"modal_contents"}>
                    <BackendErrorUI 
                        ...
                        turn_off_error = { backendError.clear } 
                    />
                    ...
                    <form className={"jobItemsCreator_form"} onSubmit={ handleSubmit }>
                        ...
                        <input type="submit" action="submit" id="items_submit_button" class="button-primary button-wide" value="submit"></input>
                    </form>
                </div>
            </div>
        </Modal>
...
function JobItemsCreatorRowRemoveButton(props){
    ...
    return <button class="delete-panel formSet_removeItemButton" onClick={ props.handle_click }><span>remove</span></button>
}
    ...
    return [
        <Modal close={props.editor.off}>
            <div>
                ...
                <div className={"modal_contents"}>
                    <div className={"formy"}>
                        <JobItemSharedFormFields    controlled = { controlled }
                                                    description = { props.data.description }
                                                    job_id = { props.job_id }
                                                    prefix = ''         
                                                    URL_GET_DATA = { props.URL_GET_DATA }
                                                    URL_ITEMS = {props.URL_ITEMS}
                                                    />
                        <BackendErrorUI message = { backendError.message }
                                        turn_off_error = { backendError.clear } />
                        <EditorControls     delete = { handleDelete }
                            submit = { handleSubmit }
                            want_delete = { true }
                        />
                    </div>
...
function useJobItemEditor(refProps){
    ...
    const backendErrorObject = get_backend_error_object(backendError, setBackendError);
    ...
    const save_item = () => {
        ...
        update_server(url, headers, resp_data => {
            if(status_is_good(resp_data, 204)){
                ...
                backendErrorObject.set(get_error_message(resp_data));
            }
        });
    };
    ...
    return {
        backendError: backendErrorObject,
        controlled,
        ...
        <div className={"formy_inputsContainer"}>
            <LabelFieldContainer>
                ...
                <input  
                    ...
                    type="number" name={ FFN_QUANTITY.name } 
                    id={ FFN_QUANTITY.id } 
                    value={ props.controlled.quantity.get }
            ...
            <LabelFieldContainer>
                ...
                <input  
                    ...
                    value={ props.controlled.selling_price.get } 
...
function JobItemAutoDescUI(props){
    ...
    return <span class='formy_productDesc'>{ props.description }</span>
}
