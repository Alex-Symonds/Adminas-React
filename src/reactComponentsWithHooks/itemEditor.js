...
// || Hook
export function useJobItemEditor(refProps){
    const { data, closeEditor, delete_item, update_item } = refProps.current;

    const [quantity, setQuantity] = useState(data.quantity);
    const [sellingPrice, setSellingPrice] = useState(data.selling_price);
    const [productId, setProductId] = useState(data.product_id);
    const [priceListId, setPriceListId] = useState(data.price_list_id);
...
// || Components
export function JobItemEditor({ closeEditor, data, delete_item, job_id, update_item }){
    ...
    return (
        <Modal close={ closeEditor }>
            <div>
                <h3 className="modal_heading">Edit Item</h3>
