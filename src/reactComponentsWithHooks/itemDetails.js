...
import { format_money, nbsp} from '../util';
import { getJobItemModularStatusCSS } from '../jobPageRoutes/items';
...
import { JobItemEditor } from './itemEditor';


// || Hook
export function useJobItemDetails(items_list){
...
// || Container
export function JobItemsDetailsContainer({ actions_items, currency, detailsKit, job_id, modalKit, product_slot_assignments, URL_MODULE_MANAGEMENT }){
    ...
    return (
        ...
        >
            <div className="jobItems_detailsHeaderStrip">
            ...
            <div className={"jobItems_detailsContent"}>
...
function JobItemReaderUI({ currency, data, job_id, openJobItemEditor, product_slot_data, URL_MODULE_MANAGEMENT }){
    return (
        <div className="jobItems_detailsJobItem">
            <JobItemHeadingUI   
...
function JobItemHeadingUI({ description, ji_id, part_number, product_name, quantity }){
    return (
        <div className="subsection">
            <h5 className="subsection_heading what">
            { quantity === 0 || quantity === null || quantity === undefined
                ? null
                : <JobItemHeadingQuantitySpan  quantity = { quantity } />
            }
                <span className="product">{ part_number }: { product_name }</span>
...
function JobItemHeadingQuantitySpan({ quantity }){
    return <span className="quantity">{ quantity } x </span>
}
...
function JobItemMoneyUI({ currency, data, openJobItemEditor }){
    ...
    return (
        <div className="money subsection">
...
function JobItemAccessoriesUI({ data }){
    ...
    return (
        <div className="std-accs-container subsection">
            <div className="std-accs">
...
function JobItemSpecificationUI({ data, URL_MODULE_MANAGEMENT }){
    ...
    return (
        <div className={'module-status-section subsection modules-' + css_module_status}>
...
function WarningMessageSpan({ message }){
    return <span className="warning-msg">
...
function JobItemsWithMatchingProductUI({ currency, members, title }){
    return (
        <div className="subsection product">
...
function JobItemAssignmentsUI({ product_slot_data }){
    return (
        <div className="module-status-section assignments subsection">
            <div className="intro-line">
                <span className="display-text">
                    &laquo; Assignment
                </span>
...
function JobItemAssignmentsCounterUI({ display_str, num, total }){
    return (
        <div className="assignment-icon">
            <span className="label">{display_str}</span>
            <span className="status">{num}{display_str === 'used' ? "/" + total : ""}</span>
        </div>
    )
}
...
function JobItemSlotFillerUI({ currency, data }){
    return (
        <div className="jobItems_detailsJobItem product">
            <JobItemHeadingUI   
                description = { data.description }
                ji_id = { null }
                part_number = { data.part_number }
...
function QuantityNameLi(props){
    return <li>{ props.quantity } x {props.name}</li>
}
