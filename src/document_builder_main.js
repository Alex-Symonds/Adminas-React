...
function update_document(issue_date){
    ...
    if(window.DOC_ID === DOC_ID_WHEN_CREATING_NEW){
        ...
        update_document_on_server('PUT', 204, obj, `id=${window.DOC_ID}`);
    }
}
...
async function update_document_on_server(method, expected_response_code, doc_data, get_params){
    ...
    if(status_is_good(resp_data, expected_response_code)){
        if(expected_response_code === 201){
            ...
            update_page_after_successful_save(`${window.URL_DOCBUILDER}?${get_params}`);
        }
    }
    else {
...
async function delete_document(){
    ...
    if(delete_confirmed){
        ...
        let resp_data = await update_backend(`${window.URL_DOCBUILDER}?id=${window.DOC_ID}`, request_options);
        if(status_is_good(resp_data, 204)){
            window.location.href = window.JOB_URL;
        }
        else{
            display_document_response_message(resp_data);
        }
...
// || Unsaved changes warning
export function show_save_warning_ele(){
